...
* The data appears to be right-skewed, with a large amount of low-count admissions (< 100)
* The mean admissions across all variables is ~45
* The maximum admissions count for any indication is 2862
* Zooming in on the lesser-occuring values, we can see that there are very few instances of admissions higher than 1000 - indicating a sharp tail at the higher end. This is likely due to population distributions across the 60+ counties in NY state.
...
annualAdmissions <- function() {
  ...
  total_admissions %>% 
    ...
    labs(title = "Total admissions per year, all of New York State",
         x = "Year",
         y = "Total Admissions",
         caption = "Admissions aggregated by year and maximum year identified") + 
    theme(
...
ggplot(top_5_admission_counties, aes(x=county_code, y = percentage)) + geom_col(fill = "blue") + 
    labs(x = "County", y= "Percentage (%)", title = "Top 5 Admission Percentage by Counties", caption = "This bar chart shows the top five counties in New York City that had the highest percentage of admissions to the chemical dependence treatment program.") + 
